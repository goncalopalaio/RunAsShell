...
elif [ "$command" = "test-input-2" ];then
        adb shell CLASSPATH=/data/local/tmp/run-as-shell.apk app_process / Runner input "8-9-10"

elif [ "$command" = "test-input-3" ];then
        adb shell CLASSPATH=/data/local/tmp/run-as-shell.apk app_process / Runner input "8-SLEEP111-9-10"
...
elif [ "$command" = "test-help" ];then
    adb shell CLASSPATH=/data/local/tmp/run-as-shell.apk app_process / Runner help
